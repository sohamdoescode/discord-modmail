//requiring dependencies- DO NOT MODIFY these unless you know what you're doing
//the settings
const {
  token,
  prefix,
...
client.on("ready", () => {
  ...
  client.user.setActivity("my DMs for help |  Z SUPPORT MODMAIL", {
    type: "LISTENING",
  });
});
...
client.on("message", async (message) => {
  ...
  if (message.channel.type === "dm") {
    ...
    //modmail thread channel does not exist 
      ...
      mailChannel.send(
        `<@&${modMailRole}> **${message.author.tag}** has created a new thread`
      );
  ...
  if (message.content === `${prefix}info`) {
    message.channel.send(
      new MessageEmbed()
        .setTitle("Z Support Modmail Bot")
        .setDescription(
          "Hi there! Z support modmail bot is an open sourced discord.js project to aid Discord servers with managing ModMails\n👉 Open sourced project developed by `zhue#5683`\n📌 I am open-sourced [here](https://github.com/zhue675/discord.js-modmail)\n💞 Join the Z-Cafe [here](https://discord.gg/J2uEYyUPu5)"
        )
        .setColor("BLUE")
        .setTimestamp()
    );
